...
def save_misclassified_examples(model, test_loader, output_folder):
    ...
    with torch.no_grad():
        for images, labels, filenames in test_loader:
            ...
            for i in range(len(images)):
                if incorrect_preds[i]:
                    image_file = filenames[i]
                    predicted_class = predictions[i].item()
                    actual_class = labels[i].item()

                    # Save the misclassified image
                    misclassified_image_path = os.path.join(output_folder, f"{misclassified_count}_pred_{predicted_class}_actual_{actual_class}.png")
                    save_image(images[i], misclassified_image_path)

                    # Append the misclassified info to the list
                    misclassified_info.append((misclassified_image_path, predicted_class, actual_class))

                    misclassified_count += 1

    # Save the misclassified info as a CSV file
    csv_file_path = os.path.join(output_folder, "misclassified_info.csv")
    df = pd.DataFrame(misclassified_info, columns=["image_file", "predicted_class", "actual_class"])
    df.to_csv(csv_file_path, index=False)

    model.train()
    print(f"{misclassified_count} misclassified examples saved in '{output_folder}'.")
...
def make_prediction(model, loader, output_csv="submission.csv"):
    ...
    for x, y, files in tqdm(loader):
        ...
        with torch.no_grad():
            #Changing to classifier
            predictions = model(x).argmax(1)
            #Changing to Regressor
#             predictions = model(x)
